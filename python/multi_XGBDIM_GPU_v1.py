...
class Dataset_global(Dataset):#全局数据集
...
class Dataset_local(Dataset): #局部数据集
    def __init__(self, x, y):
        self.x = x
        self.y = y
        self.indices = list(range(x.shape[0]))
...
class XGBDIM():
    ...
    def read_data(self, dataset):
        if dataset is not None:
            ...
            for idx_set in range(N_trainsets):
                ...
                #     )
                # )
                file_path = os.path.join(self.data_path,'sub'+str(self.sub_idx)+'A.npz')
    ...
    def train_model(self):
        ...
        self.Tset_train, self.NTset_train, self.Tset_train_global, self.NTset_train_global, self.K1, self.K2 = \
            self.get_data_ZT206_HYX_train()# self.get_data(self.trainset)
