...
XGBDIM::XGBDIM(const std::string& model_path, const std::string& rknn_model_path,
               int win_len, int chan_xlen, int chan_ylen, int step_x, int step_y,
               int max_N_model, float gstf_weight)
    : model_path(model_path), rknn_model_path(rknn_model_path),
...
std::tuple<float, float, float, float, float> XGBDIM::test() {
    ...
    std::string data_dir = "/home/hzhy/workspace/csk/RSVPStream/data/egg_data";
    ...
    for (auto file_path : file_list) {
        ...
        float s_mean = rknn_runner(x_local, x_global, rknn_model_path)[0];
